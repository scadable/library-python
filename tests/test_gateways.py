...
def test_list_gateways(client, mock_api):
    mock_api.get("/api/projects/p1/gateways").mock(return_value=Response(200, json=[
...
def test_get_gateway(client, mock_api):
    mock_api.get("/api/projects/p1/gateways/gw1").mock(return_value=Response(200, json={
        "id": "gw1", "gateway_id": "gw1", "name": "Pi 5", "status": "online",
        "version": "0.6.5", "os": "linux", "arch": "arm64",
        "devices": [
            {"id": "d1", "name": "Modbus Sensor", "status": "connected", "protocol": "modbus"},
        ],
    }))
...
def test_gateway_metrics(client, mock_api):
    mock_api.get("/api/projects/p1/gateways/gw1/metrics").mock(return_value=Response(200, json={
        "gateway_id": "gw1", "range": "1h",
...
def test_gateway_security(client, mock_api):
    mock_api.get("/api/projects/p1/gateways/gw1/security").mock(return_value=Response(200, json={
        "gateway_firmware": "0.6.5", "kernel": "6.1.0",
        "package_count": 150,
        "vulnerability_summary": {"critical": 0, "high": 2, "medium": 5, "low": 10},
