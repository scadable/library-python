...
class Gateways(SyncResource):
    ...
    def get(self, *, project_id: str, gateway_id: str) -> Gateway:
        return self._get(
            f"/api/projects/{project_id}/gateways/{gateway_id}", model=Gateway
        )

    def metrics(
        self, *, project_id: str, gateway_id: str, range: str = "1h"
    ) -> GatewayMetrics:
        return self._get(
...
class AsyncGateways(AsyncResource):
    ...
    @asynccontextmanager
    async def stream(
        self, *, project_id: str, gateway_id: str
        ...
        path = (
            f"/api/projects/{project_id}/gateways/{gateway_id}/ws"  # pragma: no cover
        )
